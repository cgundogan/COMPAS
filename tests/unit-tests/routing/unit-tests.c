...
void setUp(void)
{
    memset(compas_dodags, 0, sizeof(compas_dodags));
}

void tearDown(void) {}


compas_dodag_t compas_dodags[COMPAS_DODAG_NUMOF];

void test_compas_dodag_init_root(void)
{
    compas_dodag_t *dodag = &compas_dodags[0];
...
void test_compas_pam_parse(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT16(dodag->prefix_len, dodag2.prefix_len);
    TEST_ASSERT_EQUAL_STRING_LEN(dodag->prefix, dodag2.prefix, dodag->prefix_len);
    TEST_ASSERT_EQUAL_UINT16(dodag->rank + 1, dodag2.rank);
